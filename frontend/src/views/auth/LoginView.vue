  ...
  <div class="min-h-screen flex items-center justify-center p-4" style="background:linear-gradient(135deg,#f5f5f5,#fff1f1)">
    <div class="w-full max-w-md animate-fade-in">
      ...
      <div class="text-center mb-8">
        ...
        <h1 class="text-2xl font-black text-gray-900 mt-6">Đăng nhập</h1>
        <p class="text-gray-500 mt-1 text-sm">Chào mừng bạn trở lại Giày Đẹp Store</p>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8">
        <form @submit.prevent="handleLogin" class="space-y-5">
          ...
          <div>
            ...
            <div class="relative">
              <input v-model="password" :type="showPw ? 'text' : 'password'" required class="input-field pr-10" placeholder="••••••••" />
        ...
        <div class="mt-6 text-center">
          <p class="text-sm text-gray-500">
            Chưa có tài khoản?
            <RouterLink to="/register" class="font-bold hover:underline" style="color:#e8191a">Đăng ký ngay</RouterLink>
          </p>
        </div>

        <!-- Demo accounts -->
        <div class="mt-5 p-4 bg-gray-50 rounded-lg">
          <p class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Tài khoản demo:</p>
          <button @click="fillAdmin" class="text-xs font-semibold hover:underline block" style="color:#e8191a">
            👑 Admin: admin@qlbg.com / admin123
          </button>
        </div>
      </div>
...
async function handleLogin() {
  ...
  try {
    ...
    errorMsg.value = err.response?.data?.detail || 'Email hoặc mật khẩu không đúng'
  } finally {
    loading.value = false
  }
}
